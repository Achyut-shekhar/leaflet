...
function AirTrafficMap() {
  ...
  const fetchAirTraffic = async () => {
    ...
    try {
      ...
      if (error.message.includes("Rate limited") || error.message.includes("429")) {
        ...
        setError(`Rate limited by OpenSky API. Using sample data. Will retry at ${retryDate.toLocaleTimeString()}`);
        ...
        // Other errors
        setError(`Error fetching flight data: ${error.message}. Using sample data.`);
        setUseSampleData(true);
      }
    } finally {
      setLoading(false);
      ...
      if (currentMarkers[icao24]) {
        ...
        marker.bindPopup(`
          <div style="min-width: 200px;">
            <h3 style="font-weight: bold; font-size: 16px; margin-bottom: 8px;">${plane.callsign}</h3>
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                ...
                <td style="padding: 3px 0;">${Math.round(plane.altitude).toLocaleString()} m</td>
              </tr>
              <tr>
                <td style="padding: 3px 0;"><strong>Speed:</strong></td>
                <td style="padding: 3px 0;">${Math.round(plane.velocity).toLocaleString()} m/s</td>
              </tr>
              <tr>
  ...
  const toggleDataSource = () => {
    ...
    if (useSampleData) {
      ...
      if (nextRetryTime && nextRetryTime > new Date()) {
        alert(`API is rate limited. Next retry scheduled at ${nextRetryTime.toLocaleTimeString()}`);
        return;
      }
  ...
  return (
    <div className="flex flex-col space-y-4">
      <h2 className="text-center font-bold text-2xl">Live Air Traffic Map - India</h2>
      ...
      {/* Status bar */}
      <div className="flex flex-col md:flex-row justify-between items-center p-3 bg-gray-100 rounded-md">
        <div className="flex flex-col">
          <div className="flex items-center space-x-2">
            {loading ? (
              ...
              <div className="flex items-center">
                <div className={`w-3 h-3 rounded-full mr-2 ${useSampleData ? "bg-amber-500" : "bg-green-500"}`}></div>
          ...
          <div className="text-sm mt-1">
            ...
            <span className="font-medium ml-2">Last updated:</span> {formatLastUpdated()}
          </div>
      ...
      {/* Legend */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 p-3 bg-white rounded-md shadow-sm text-sm">
        <div className="flex items-center">
      ...
      <div className="p-3 bg-blue-50 rounded-md text-sm border border-blue-200">
        <p>
          <strong>Note:</strong> The OpenSky Network API has strict rate limits for free accounts. 
          If you see "Too many requests" errors, the app will automatically switch to sample data 
          and try again later. For unlimited access, consider using an OpenSky Network account.
        </p>
      </div>
    </div>
  );
